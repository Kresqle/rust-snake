...
impl Game {
    ...
    fn pressed(&mut self, btn: &Button) {
        let last_direction: Direction = self.snake.d.clone();
...
impl Snake {
    pub fn render(&mut self, args: &RenderArgs) {
        ...
        let squares: Vec<graphics::types::Rectangle> = self.snake_parts
            .iter()
            .map(|p: &Snake_Piece| Snake_Piece(p.0 * self.width, p.1 * self.width))
            .map(|p: Snake_Piece| graphics::rectangle::square(p.0 as f64, p.1 as f64, self.width as f64))
            .collect();

        self.gl.draw(args.viewport(), |c: Context, gl: &mut GlGraphics| {
            let transform: [[f64; 3]; 2] = c.transform;

            squares
                .into_iter()
                .for_each(|square: [f64; 4]| graphics::rectangle(RED, square, transform, gl));
        })
    }
    ...
    fn is_collide(&self, x: u32, y: u32) -> bool {
        self.snake_parts.iter().any(|p: &Snake_Piece| x == p.0 && y == p.1)
    }
}
...
impl Food {
    fn update(&mut self, s: &Snake) -> bool {
        let front: &Snake_Piece = s.snake_parts.front().unwrap();
    ...
    fn render(&mut self, gl: &mut GlGraphics, args: &RenderArgs, width: u32) {
        ...
        let x: u32 = self.x * width;
        let y: u32 = self.y * width;

        let square: [f64; 4] = graphics::rectangle::square(x as f64, y as f64, width as f64);
...
fn main() {
    ...
    let WIDTH: u32 = COLS * SQUARE_WIDTH;
    let HEIGHT: u32 = ROWS * SQUARE_WIDTH;
    ...
    let mut events: Events = Events::new(EventSettings::new()).ups(10);
